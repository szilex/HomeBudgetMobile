package edu.michaelszeler.homebudget.HomeBudgetMobile.ui.dialog
...
class BudgetSummaryDialogFragment : DialogFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val fullAngle = BigDecimal(360)
        val sum = customExpenses.plus(regularExpenses).plus(strategies)
        val angles = listOf(customExpenses.divide(sum,2, RoundingMode.HALF_UP).multiply(fullAngle),regularExpenses.divide(sum,2, RoundingMode.HALF_UP).multiply(fullAngle),strategies.divide(sum,2, RoundingMode.HALF_UP).multiply(fullAngle))
