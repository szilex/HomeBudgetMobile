...
class StrategiesTabFragment(private val strategies : List<StrategyEntry>) : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.recycler_view_strategies_tab.setHasFixedSize(true)
        val gridLayoutManager = GridLayoutManager(context, 1, RecyclerView.VERTICAL, false)

        view.recycler_view_strategies_tab.layoutManager = gridLayoutManager
        val adapter = StrategyCardRecyclerViewAdapter(strategies, fragmentManager)
        view.recycler_view_strategies_tab.adapter = adapter
        ...
        view.recycler_view_strategies_tab.addItemDecoration(StrategyGridItemDecoration(largePadding, smallPadding))
