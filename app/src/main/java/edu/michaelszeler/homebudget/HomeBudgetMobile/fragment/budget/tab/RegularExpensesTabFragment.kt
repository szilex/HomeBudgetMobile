...
class RegularExpensesTabFragment(private val expenses : List<RegularExpenseEntry>) : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.recycler_view_regular_expenses_tab.setHasFixedSize(true)
        val gridLayoutManager = GridLayoutManager(context, 1, RecyclerView.VERTICAL, false)

        view.recycler_view_regular_expenses_tab.layoutManager = gridLayoutManager
        val adapter = RegularExpenseCardRecyclerViewAdapter(expenses, fragmentManager)
        view.recycler_view_regular_expenses_tab.adapter = adapter
        ...
        view.recycler_view_regular_expenses_tab.addItemDecoration(StrategyGridItemDecoration(largePadding, smallPadding))
