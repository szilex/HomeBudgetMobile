package edu.michaelszeler.homebudget.HomeBudgetMobile.view.fragment.budget
...
class ArchiveBudgetsFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val jsonArrayRequest = object: JsonArrayRequest(
                Method.GET,
                "http://10.0.2.2:8080/budget/archive",
                null,
                {
                    response: JSONArray? ->
                    run {
                        ...
                        view.recycler_view_archive_budgets.addItemDecoration(CustomGridItemDecoration(largePadding, smallPadding))

                        view.relative_layout_archive_budgets.visibility = View.VISIBLE
                        view.loading_panel_archive_budgets.visibility = View.GONE
                    }
                },
                {
                    error: VolleyError? ->
                        ...
                        if (networkResponse?.statusCode == 401) {
                            ...
                            if (answer.has("message")) {
                                ...
                                when (answer.getString("message")) {
                                    ...
                                    "no budgets found" -> {
                                        Toast.makeText(activity, "Could not find any archive budgets", Toast.LENGTH_SHORT).show()
                                    }
                                    else -> {
