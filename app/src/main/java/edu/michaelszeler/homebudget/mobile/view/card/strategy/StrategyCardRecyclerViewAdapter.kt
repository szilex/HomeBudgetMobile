...
import edu.michaelszeler.homebudget.mobile.view.listener.ShowStrategyChartOnClickListener
import java.util.*
...
class StrategyCardRecyclerViewAdapter(private val strategyList: List<StrategyEntry>, private val fragmentManager: FragmentManager?) : RecyclerView.Adapter<StrategyCardViewHolder>() {
    ...
    override fun onBindViewHolder(holder: StrategyCardViewHolder, position: Int) {
        if (position < strategyList.size) {
            ...
            holder.strategyGoal.text = String.format("%10.2f", product.goal)
            ...
            holder.strategyShowChartButton.setOnClickListener(ShowStrategyChartOnClickListener(fragmentManager!!, product.goal.toDouble(), product.months, calculateCurrentMonth(startCalendar)))
